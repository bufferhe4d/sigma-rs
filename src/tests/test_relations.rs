...
use crate::linear_relation::{LinearRelation, Sum};
...
pub fn discrete_logarithm<G: PrimeGroup, R: rand::RngCore>(
    ...
    let var_x = instance.allocate_scalar();
    let var_G = instance.allocate_element();

    let var_X = instance.allocate_eq(var_x * var_G);

    instance.set_element(var_G, G::generator());
    instance.compute_image(&[x]).unwrap();

    let X = instance.linear_map.group_elements.get(var_X).unwrap();
...
#[allow(non_snake_case)]
pub fn shifted_dlog<G: PrimeGroup, R: RngCore>(rng: &mut R) -> (LinearRelation<G>, Vec<G::Scalar>) {
    let x = G::Scalar::random(rng);
    let mut instance = LinearRelation::new();
    ...
    let var_X = instance.allocate_eq(var_G * var_x + var_G * <G::Scalar as Field>::ONE);
    // another way of writing this is:
    instance.append_equation(var_X, (var_x + G::Scalar::from(1)) * var_G);
...
#[allow(non_snake_case)]
pub fn dleq<G: PrimeGroup, R: RngCore>(rng: &mut R) -> (LinearRelation<G>, Vec<G::Scalar>) {
    ...
    let var_X = instance.allocate_eq(var_x * var_G);
    let var_Y = instance.allocate_eq(var_x * var_H);

    instance.set_elements([(var_G, G::generator()), (var_H, H)]);
    instance.compute_image(&[x]).unwrap();

    let X = instance.linear_map.group_elements.get(var_X).unwrap();
    let Y = instance.linear_map.group_elements.get(var_Y).unwrap();
...
#[allow(non_snake_case)]
pub fn shifted_dleq<G: PrimeGroup, R: RngCore>(rng: &mut R) -> (LinearRelation<G>, Vec<G::Scalar>) {
    let H = G::random(&mut *rng);
    let x = G::Scalar::random(&mut *rng);
    let mut instance = LinearRelation::new();
    ...
    let var_X = instance.allocate_eq(var_x * var_G + var_H);
    let var_Y = instance.allocate_eq(var_x * var_H + var_G);
...
pub fn pedersen_commitment<G: PrimeGroup, R: RngCore>(
    ...
    instance.set_elements([(var_H, H), (var_G, G::generator())]);
    instance.compute_image(&[x, r]).unwrap();

    let C = instance.linear_map.group_elements.get(var_C).unwrap();
...
pub fn pedersen_commitment_dleq<G: PrimeGroup, R: RngCore>(
    ...
    let var_Gs = instance.allocate_elements::<4>();
    let var_X = instance.allocate_eq(var_x * var_Gs[0] + var_r * var_Gs[1]);
    let var_Y = instance.allocate_eq(var_x * var_Gs[2] + var_r * var_Gs[3]);
    ...
    instance.set_elements([(var_X, X), (var_Y, Y)]);

    assert!(vec![X, Y] == instance.linear_map.evaluate(&witness).unwrap());
...
pub fn bbs_blind_commitment<G: PrimeGroup, R: RngCore>(
    ...
    let [var_secret_prover_blind, var_msg_1, var_msg_2, var_msg_3] = instance.allocate_scalars();

    // Match Sage's allocation order: allocate all elements in the same order
    let [var_Q_2, var_J_1, var_J_2, var_J_3] = instance.allocate_elements();
    let var_C = instance.allocate_element(); // Allocate var_C separately, giving it index 4
    ...
    assert!(vec![C] == instance.linear_map.evaluate(&witness).unwrap());
    (instance, witness)
}
...
pub fn weird_linear_combination<G: PrimeGroup, R: RngCore>(
    ...
    let gen__disj1_x_r_var = instance.allocate_scalar();
    let A = instance.allocate_element();
    let var_B = instance.allocate_element();

    let sigma__eq1 = instance.allocate_eq(A * G::Scalar::from(1) + gen__disj1_x_r_var * var_B);

    // Set the group elements
    instance.set_elements([(A, G::generator()), (var_B, B)]);
    instance.compute_image(&[gen__disj1_x_r]).unwrap();

    let result = instance.linear_map.group_elements.get(sigma__eq1).unwrap();
...
fn simple_subtractions<G: PrimeGroup, R: RngCore>(
    ...
    let var_x = instance.allocate_scalar();
    let var_B = instance.allocate_element();
    let var_X = instance.allocate_eq((var_x + (-G::Scalar::from(1))) * var_B);
    instance.set_element(var_B, B);
    instance.set_element(var_X, X);
...
fn subtractions_with_shift<G: PrimeGroup, R: RngCore>(
    ...
    let var_x = instance.allocate_scalar();
    let var_B = instance.allocate_element();
    let var_X = instance.allocate_eq((var_x + (-G::Scalar::from(1))) * var_B + (-var_B));
...
fn cmz_wallet_spend_relation<G: PrimeGroup, R: RngCore>(
    ...
    let var_n_balance = instance.allocate_scalar();
    let var_i_price = instance.allocate_scalar();
    let var_z_w_balance = instance.allocate_scalar();

    let var_P_W = instance.allocate_element();
    ...
    instance.set_elements([(var_P_W, P_W), (var_A, A)]);
...
fn test_relations() {
    ...
    let instance_generators: Vec<(
        _,
        &'static dyn Fn(&mut _) -> (LinearRelation<G>, Vec<Scalar>),
    )> = vec![
    ...
    for (relation_name, relation_sampler) in instance_generators.iter() {
        ...
        let domain_sep = format!("test-fiat-shamir-{relation_name}")
            ...
            .to_vec();
        let nizk = instance
            .into_nizk(&domain_sep)
            .expect(&format!("Failed to create NIZK for {relation_name}"));
